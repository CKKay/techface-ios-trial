...
@interface SelectClientViewController ()
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
	[userDefaults setObject:[array[indexPath.row] valueForKeyPath:@"id"] forKey:@"selected_client_id"];
	[userDefaults setObject:itemIndex forKey:@"selected_client_row"];
    [userDefaults removeObjectForKey:@"client_exposure_setting"];
	[userDefaults setObject:itemsData forKey:@"selected_client"];
	[userDefaults setObject:@"existing" forKey:@"capture_current"];
	[userDefaults synchronize];
/*
	if (![NSString isEmpty:self.searchText]) {
		DLog(@"Filtered list");
		DLog(@"selected item : %@", [self.filteredItemsDict valueForKeyPath:@"id"][indexPath.row]);
		[userDefaults setObject:[self.filteredItemsDict valueForKeyPath:@"id"][indexPath.row] forKey:@"selected_client_id"];
		NSData *myData = [NSKeyedArchiver archivedDataWithRootObject:self.filteredItemsDict];
		[userDefaults setObject:[NSNumber numberWithInteger:indexPath.row] forKey:@"selected_client_row"];
		[userDefaults setObject:myData forKey:@"selected_client"];
	} else {
		DLog(@"Full list");
		DLog(@"selected item : %@", [self.itemsDict valueForKeyPath:@"id"][indexPath.row]);
		[userDefaults setObject:[self.itemsDict valueForKeyPath:@"id"][indexPath.row] forKey:@"selected_client_id"];
		NSData *myData = [NSKeyedArchiver archivedDataWithRootObject:self.itemsDict];
		[userDefaults setObject:[NSNumber numberWithInteger:indexPath.row] forKey:@"selected_client_row"];
		[userDefaults setObject:myData forKey:@"selected_client"];
	}
*/
	// Use programmatic instead of segue
	[self performSegueWithIdentifier:@"Select Practitioner" sender:self];
	// UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
	// PractitionerSearchViewController *vc = [storyboard instantiateViewControllerWithIdentifier:@"addPractitioner"];
	// [self presentViewController:vc animated:true completion:nil];
	/*
	 [[NSNotificationCenter defaultCenter] postNotificationName:@"selectpract" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:110] forKey:@"practid"]];
	 */
}
